...
func parsePNGpHYs(r *bufio.Reader) (float64, float64, bool) {
	...
	for {
		...
		// Skip chunk data + CRC
		toSkip := length + 4
		if _, err := r.Discard(toSkip); err != nil {
			return 0, 0, false
		}
		if ctype == "IEND" {
			break
...
func detectUniformMargins(img image.Image) (int, int, int, int) {
	b := img.Bounds()
	w := b.Dx()
	h := b.Dy()

	// Helper to get RGBA at pixel
	rgbaAt := func(x, y int) (uint32, uint32, uint32, uint32) {
