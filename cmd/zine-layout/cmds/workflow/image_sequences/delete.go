...
type imageSequencesDeleteCommand struct {
	*cmds.CommandDescription
}

type imageSequencesDeleteSettings struct {
	DataRoot   string `glazed.parameter:"data-root"`
	SequenceID string `glazed.parameter:"sequence-id"`
}

func (c *imageSequencesDeleteCommand) RunIntoGlazeProcessor(
	ctx context.Context,
	parsedLayers *layers.ParsedLayers,
	gp middlewares.Processor,
) error {
	settings := &imageSequencesDeleteSettings{}
	...
	if err := repos.ImageSequences.Delete(settings.SequenceID); err != nil {
		return err
	}
...
func newImageSequencesDeleteCommand() (*cobra.Command, error) {
	glazedLayer, err := settings.NewGlazedParameterLayers()
	if err != nil {
		return nil, err
	}

	cmd := &imageSequencesDeleteCommand{
...
var _ cmds.GlazeCommand = &imageSequencesDeleteCommand{}
