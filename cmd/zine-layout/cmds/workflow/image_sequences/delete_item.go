...
type imageSequencesDeleteItemCommand struct {
	*cmds.CommandDescription
}

type imageSequencesDeleteItemSettings struct {
...
func (c *imageSequencesDeleteItemCommand) RunIntoGlazeProcessor(
	ctx context.Context,
	parsedLayers *layers.ParsedLayers,
	gp middlewares.Processor,
) error {
	settings := &imageSequencesDeleteItemSettings{}
...
func newImageSequencesDeleteItemCommand() (*cobra.Command, error) {
	glazedLayer, err := settings.NewGlazedParameterLayers()
	if err != nil {
		return nil, err
	}

	cmd := &imageSequencesDeleteItemCommand{
		CommandDescription: cmds.NewCommandDescription(
			"delete-item",
			cmds.WithShort("Remove an item from a sequence"),
			cmds.WithFlags(
...
var _ cmds.GlazeCommand = &imageSequencesDeleteItemCommand{}
