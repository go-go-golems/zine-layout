...
export const ProjectValidationPanel: React.FC<{ id: string }> = ({ id }) => {
  ...
  const onRun = () => { trigger({ id }) }

  return (
    <section>
      <h2>Validation</h2>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <button onClick={onRun} disabled={isFetching}>Run validation</button>
      ...
      {!isUninitialized && data && (
        <div style={{ marginTop: 8 }}>
          ...
          {data.details && (
            <pre style={{ background: '#f7f7f7', padding: 8, overflow: 'auto' }}>{JSON.stringify(data.details, null, 2)}</pre>
          )}
        </div>
      )}
    </section>
