...
func DoProjectRender(projectsRoot, id string, test, testBW bool, testDimensions string) (*Result, error) {
    ...
    if test {
        ...
        p, err := projects.ReadProject(projectsRoot, id)
        if err != nil { return nil, err }
        order := p.Order
        if len(order) == 0 { order = p.Images }
        files := make([]string, 0, len(order))
        for _, name := range order {
            files = append(files, filepath.Join(projects.ProjectImagesDir(projectsRoot, id), name))
        }
...
func ListProjectRenders(projectsRoot, id string) ([]ListItem, error) {
    ...
    if err != nil { if os.IsNotExist(err) { return []ListItem{}, nil }; return nil, err }
    var out []ListItem
