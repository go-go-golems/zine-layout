...
func (s *ImpositionService) ImposeZine(zineID string, presetID string) ([]*SheetResult, error) {
    ...
    if err != nil { return nil, fmt.Errorf("fetch zine: %w", err) }
    pages, err := s.repos.Zines.GetPages(zineID)
    if err != nil { return nil, fmt.Errorf("fetch zine pages: %w", err) }
    if len(pages) == 0 { return nil, fmt.Errorf("zine has no pages") }
    ...
    for _, zp := range pages {
        ...
        if err != nil { return nil, fmt.Errorf("fetch laid-out page %s: %w", zp.LaidOutPageID, err) }
        rendered, err := ps.RenderPage(page.ID)
        if err != nil { return nil, fmt.Errorf("render page %s: %w", page.ID, err) }
        if rendered.ResultJSON == nil { return nil, fmt.Errorf("page %s missing render metadata", page.ID) }
        ...
        if rel == "" { return nil, fmt.Errorf("no usable variant for page %s", page.ID) }
        abs := filepath.Join(s.dataRoot, filepath.FromSlash(rel))
        f, err := os.Open(abs)
        if err != nil { return nil, fmt.Errorf("open page image %s: %w", abs, err) }
        img, _, err := image.Decode(f)
        _ = f.Close()
        if err != nil { return nil, fmt.Errorf("decode page image %s: %w", abs, err) }
    ...
    if err != nil { return nil, fmt.Errorf("read preset %s: %w", presetID, err) }
    ...
    for idx, op := range layout.OutputPages {
        ...
        if err != nil { return nil, fmt.Errorf("render sheet %d: %w", idx, err) }
        b := out.Bounds()
...
func (s *ImpositionService) SaveSheetsAsPNGs(projectID, zineID string, sheets []*SheetResult) ([]string, error) {
    if s.dataRoot == "" { return nil, fmt.Errorf("dataRoot not configured") }
    ...
    for _, sheet := range sheets {
        ...
        if err := png.Encode(fp, sheet.Image); err != nil { _ = fp.Close(); return nil, err }
        _ = fp.Close()
